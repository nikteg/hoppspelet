"use strict";
  ...
  const MEL_A = [
    0, _, 4, _, 7, _, 9, _,      9, _, 7, _, 4, _, 2, _,      // C  Am
    5, _, 9, _, 12, _, 9, _,     7, _, 11, _, 7, _, 2, _,     // F  G
    0, _, 4, _, 7, _, 12, _,     9, _, 12, _, 16, _, 12, _,   // C  Am
    17, _, 16, _, 12, _, 9, _,   7, _, 11, _, 14, _, 11, _    // F  G
  ];
  // Versvariation - tatare "svar" pa versen
  const MEL_A2 = [
    16, 14, 12, _, 7, _, 4, _,   9, _, 12, 14, 16, _, 12, _,  // C  Am
    17, _, 12, _, 9, 12, 5, _,   7, 9, 11, _, 14, _, 11, 9,   // F  G
    12, _, 16, _, 12, _, 7, _,   9, 12, 16, _, 14, 12, 9, _,  // C  Am
    5, _, 9, _, 12, _, 16, 14,   12, 11, 7, _, 11, _, 14, _   // F  G
  ];
  // Refrang - hogre lage, mer driv
  const MEL_B = [
    12, _, 17, _, 16, _, 12, _,  14, _, 19, _, 16, _, 14, _,  // F  G
    16, _, 19, _, 21, _, 19, _,  16, _, 14, _, 12, _, 9, _,   // C  Am
    17, 16, 17, _, 21, _, 19, 17, 19, _, 14, _, 11, _, 14, _, // F  G
    16, _, 12, _, 19, _, 16, _,  14, 12, 11, _, 14, _, 19, _  // C  G
  ];
  // Brygga - gles och drommande
  const MEL_C = [
    9, _, _, _, 12, _, _, _,     7, _, _, _, 11, _, _, _,     // Am Em
    5, _, _, _, 9, _, _, _,      7, _, _, _, 11, _, 14, _,    // F  G
    16, _, _, _, 12, _, _, _,    14, _, _, _, 11, _, _, _,    // Am Em
    12, _, _, _, 9, _, _, _,     11, _, 14, _, 11, _, 7, _    // F  G
  ];

  // Ackord per takt: [basrot (halvtoner fran C5, oktaven under), ters (4=dur, 3=moll)]
  const PROG_A = [[-12, 4], [-15, 3], [-19, 4], [-17, 4], [-12, 4], [-15, 3], [-19, 4], [-17, 4]]; // C Am F G x2
  const PROG_B = [[-19, 4], [-17, 4], [-12, 4], [-15, 3], [-19, 4], [-17, 4], [-12, 4], [-17, 4]]; // F G C Am / F G C G
  const PROG_C = [[-15, 3], [-20, 3], [-19, 4], [-17, 4], [-15, 3], [-20, 3], [-19, 4], [-17, 4]]; // Am Em F G x2
